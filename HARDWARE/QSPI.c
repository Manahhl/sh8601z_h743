...
void QSPI_Init(void)
{
	...
	QSPI_Handler.Init.ChipSelectHighTime=QSPI_CS_HIGH_TIME_2_CYCLE; //片选为高延时
	...
	QSPI_Handler.Init.FifoThreshold=128;								//配置FIFO中字节阈值（仅在间接模式使用）
	...
	QSPI_CmdInitStructure.DummyCycles 		= 4;
	...
	QSPI_CmdInitStructure.SIOOMode    	 	= QSPI_SIOO_INST_ONLY_FIRST_CMD;
	QSPI_CmdInitStructure.AlternateByteMode = QSPI_ALTERNATE_BYTES_NONE;
	QSPI_CmdInitStructure.DdrMode      		= QSPI_DDR_MODE_ENABLE;
	QSPI_CmdInitStructure.DdrHoldHalfCycle 	= QSPI_DDR_HHC_HALF_CLK_DELAY;
	...
	hmdma.Init.Endianness =  MDMA_LITTLE_WORD_ENDIANNESS_EXCHANGE;
	...
	HAL_NVIC_SetPriority(MDMA_IRQn,0x01,0);
